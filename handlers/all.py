    ...
    if text == c.ID and not redis.sismember("{}Nbot:IDSend".format(BOT_ID),chatID) and not message.reply_to_message:
      ...
      if text == "الاسرع" or text == "ترتيب" :
        if redis.sismember("{}Nbot:Ugames".format(BOT_ID),chatID):
          ...
          Bot("sendMessage",{"chat_id":chatID,"text":f"🔰┇اسرع واحد يرتبها ~ {name}","reply_to_message_id":message.message_id,"parse_mode":"html"})
          if text == name:
            Bot("sendMessage",{"chat_id":chatID,"text":f"🎁┇الف مبروك لقد فزت \n♻┇للعب مره اخرى ارسل ~{ الاسرع , ترتيب }","reply_to_message_id":message.message_id,"parse_mode":"html"})
