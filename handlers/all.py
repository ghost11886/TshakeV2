...
def allGP(client, message,redis):
  ...
  if text :
    ...
    if re.search(redis.get("{}Nbot:Klmspeed".format(BOT_ID)), text):
    ...
    if text and re.search("سمايلات",text) and not redis.sismember("{}Nbot:gpgames".format(BOT_ID),chatID):
      ...
      Bot("sendMessage",{"chat_id":chatID,"text":f"🔰┇ اسرع واحد يرسل هذا السمايل : {SM}  يربح 4 نقاط","reply_to_message_id":message.message_id,"parse_mode":"html","disable_web_page_preview":True})
   
    if re.search(redis.get("{}Nbot:smiles".format(BOT_ID)), text):
    ...
    if re.search(redis.get("{}Nbot:Hzor".format(BOT_ID)), text):
    ...
    if re.search(redis.get("{}Nbot:MaanyRand".format(BOT_ID)), text):
    ...
    if re.search(redis.get("{}Nbot:opissite".format(BOT_ID)), text):
    ...
    if re.search(redis.get("{}Nbot:amthila".format(BOT_ID)), text):
